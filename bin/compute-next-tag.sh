...
#
# Tags look like `v<cAdvisor version>-<release>`:
#
# - if defaults/main.yml points at a cAdvisor version that has never been
#   released, the release counter restarts at 0 (`v0.55.2-0`)
# - otherwise the counter is incremented (`v0.55.2-1`), but only if something
...
# annotation above the value nor the variables derived from it
# (`cadvisor_container_image_tag`, for one) can be picked up instead.
version="$(sed -nE 's|^cadvisor_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the cAdvisor version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v0.55.1`),
